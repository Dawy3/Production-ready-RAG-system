...
async def index_project(request: Request, project_id: int, push_request: PushRequestSchema):
    ...
    # setup batching 
    total_chunks_count = await chunk_model.get_total_chunks(project_id=project.project_id)
    pbar = tqdm(total=total_chunks_count, desc="Vector Indexing", position=0)
    
    
    
    while has_records:
        page_chunks = await chunk_model.get_project_chunks(project_id=project_id, page_number=page_number)
        if len(page_chunks):
            page_number +=1
